<div>
    @php
        ...
        if (Auth::user() && Auth::user()->hasPower('manage_characters') && Auth::user()->hasPower('view_hidden_genetics')) {
            ...
            $button .= '></i></a>';
            if ($genome) {
                $button .= "<a href=\"#\" class=\"btn btn-link btn-sm text-danger delete-genome\" data-genome-id=\"" . $genome->id . "\"><i class=\"fas fa-minus\"></i></a>";
            }
        }
    @endphp
    ...
    @else
        @php
            ...
            if (Auth::user() && Auth::user()->hasPower('view_hidden_genetics')) {
                $visible = 2;
            } elseif (isset($genome->visibility_level)) {
                $visible = $genome->visibility_level;
            } else {
                $visible = Settings::get('genome_default_visibility');
            }
        @endphp
        @if ($visible < 1)
