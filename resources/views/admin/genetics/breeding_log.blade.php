                ...
                <div class="row justify-content-center">
                    @foreach ($log->parents as $parent)
                        <div class="col-md-3 col-6 text-center mb-3">
                            ...
                            <div class="mt-1">
                                <a href="{{ $parent->character->url }}" class="h5 mb-0">
                                    @if (!$parent->character->is_visible)
                                        <i class="fas fa-eye-slash"></i>
                                    @endif {{ $parent->character->fullName }}
                                </a>
                            </div>
                        </div>
                    @endforeach
                            ...
                            <div class="mt-1">
                                <a href="{{ $child->character->url }}" class="h5 mb-0">
                                    @if (!$child->character->is_visible)
                                        <i class="fas fa-eye-slash"></i>
                                    @endif {{ $child->character->fullName }}
                                </a>
                                @if ($child->twin)
                                    <br>Twin of {!! $child->twin->displayName !!}
                                @endif
                                @if ($child->chimerism)
                                    <br><strong>Chimera</strong>
                                @endif
                            </div>
                        </div>
                    @endforeach
