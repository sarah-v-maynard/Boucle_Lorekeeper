...
class CharacterGenome extends Model {
    ...
    **********************************************************************************************/
    ...
    public function getLoci() {
        ...
        foreach (array_unique($this->gradients->pluck('loci_id')->toArray()) as $value) {
            array_push($array, $value);
        }
        foreach (array_unique($this->numerics->pluck('loci_id')->toArray()) as $value) {
            array_push($array, $value);
        }
            ...
            if ($this->hasLocus($loci) || $sire->hasLocus($loci)) {
                ...
                if ($loci->type == 'gene') {
                    ...
                    for ($i = 0; $i < $loci->length / 2; $i++) {
                        $rufus .= substr($dam, ($i * 2) + mt_rand(0, 1), 1).substr($pop, ($i * 2) + mt_rand(0, 1), 1);
                    }
                    ...
                    if ($genes[0] === null && $loci->default != 1) {
                        $num = $genes[1];
                    }
                    if ($genes[1] === null && $loci->default != 1) {
                        $num = $genes[0];
                    }
                    if ($num) {
                ...
                if ($flag) {
                    array_push($genodata['gene_id'], $loci->id);
                }
            }
        }
    ...
    public function crossWith($sire) {
        ...
        foreach (Loci::query()->orderBy('sort', 'desc')->get() as $loci) {
            if ($this->hasLocus($loci) || $sire->hasLocus($loci)) {
                ...
                if ($gene !== null) {
                    array_push($genes, "<span class='text-monospace text-nowrap mx-1' data-toggle='tooltip' title='".$loci->name."'>".$gene.'</span>');
                }
            }
        }
    ...
    private function inheritGene($loci, $damGenes, $sireGenes) {
        if ($loci->type == 'gene') {
            ...
            // Matrilineal gene
            if ($damGenes == null) {
                $alleles .= $loci->getDefault()->display_name;
            } else {
                $alleles .= $damGenes->random()->allele->display_name;
            }
            ...
            if ($sireGenes == null) {
                ...
                $alleles .= $sireGenes->random()->allele->display_name;
            }
            ...
            if ($damGenes == null) {
                ...
                $dam = $damGenes->random()->display_genome;
            }

            // Patrilineal gene
            if ($sireGenes == null) {
                $sire = $loci->getDefault();
            } else {
                $sire = $sireGenes->random()->display_genome;
            }
