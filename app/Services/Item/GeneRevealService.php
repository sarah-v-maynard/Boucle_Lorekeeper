...
class GeneRevealService extends Service {
    ...
    */
    ...
    public function act($stacks, $user, $data) {
        DB::beginTransaction();
        try {
            if (Settings::get('genome_default_visibility') == 2) {
                throw new \Exception('Genomes are fully visible. Gene reveals are useless here.');
            }
            if ($stacks->count() > 1 || $data['quantities'][0] > 1) {
                throw new \Exception("That's too many gene reveals! You can only use these one at a time.");
            }
            $stack = $stacks->first();
            ...
            if (!$genome) {
                throw new \Exception("Couldn't find that genome.");
            }
            if ($genome->character->user->id != $user->id) {
                throw new \Exception("Cannot gene reveal a character that isn't yours.");
            }

            $tagData = $stack->item->tag('gene_reveal')->getData();
            $vis = max($genome->visibility_level, Settings::get('genome_default_visibility'));
            if ($tagData['fully_hidden_only'] && $vis != 0) {
                throw new \Exception('This item can only be used on fully hidden genomes');
            }
